...
payload = {'id_client': ID_client}
r = requests.get(
    'https://projetflask.herokuapp.com/get_score_credit', params=payload)
...
model_LGBM = pickle.load(open("model_LGBM.pkl", "rb"))
y_pred_lgbm = model_LGBM.predict(X_test_clean().drop(
    labels="sk_id_curr", axis=1))    # Prédiction de la classe 0 ou 1
y_pred_lgbm_proba = model_LGBM.predict_proba(X_test_clean().drop(
    labels="sk_id_curr", axis=1))  # Prédiction du % de risque

# Récupération du score du client
y_pred_lgbm_proba_df = pd.DataFrame(y_pred_lgbm_proba, columns=[
...
y_pred_lgbm_df = pd.concat(
    [y_pred_lgbm_df, X_test_clean()['sk_id_curr']], axis=1)
y_pred_lgbm_df['client'] = np.where(
    y_pred_lgbm_df.prediction == 1, "non solvable", "solvable")
y_pred_lgbm_df['decision'] = np.where(
    y_pred_lgbm_df.prediction == 1, "refuser", "accorder")
solvabilite = y_pred_lgbm_df.loc[y_pred_lgbm_df['sk_id_curr']
                                 == ID_client, "client"].values
decision = y_pred_lgbm_df.loc[y_pred_lgbm_df['sk_id_curr']
                              == ID_client, "decision"].values
...
with col2:
    ...
    st.write(
        f"Le client dont l'identité numérique est {ID_client} a obtenu le score de {score }.")
    st.write(
        f"Il y a donc un risque de {score:} que le client ait des difficultés de paiement.")
...
st.write(
    f"Ancienneté de la piède d'identité : **{X_test_original()['anciennete_cni'].values[0]}**")
st.write(
    f"Situation familiale : **{X_test_original()['name_family_status'].values[0]}**")
st.write(
    f"Taille de la famille : **{X_test_original()['taille_famille'].values[0]}**")
st.write(
    f"Nombre d'enfants : **{X_test_original()['nbr_enfants'].values[0]}**")
st.write(
    f"Niveau d'éducation : **{X_test_original()['name_education_type'].values[0]}**")
st.write(
    f"Revenu Total Annuel : **{X_test_original()['total_revenus'].values[0]} $**")
st.write(
    f"Type d'emploi : **{X_test_original()['name_income_type'].values[0]}**")
st.write(
    f"Ancienneté dans son entreprise actuelle : **{X_test_original()['anciennete_entreprise'].values[0]}**")
st.write(
    f"Type d'habitation : **{X_test_original()['name_housing_type'].values[0]}**")
st.write(
    f"Densité de la Population de la région où vit le client : **{X_test_original()['pop_region'].values[0]}**")
...
st_shap(shap.decision_plot(explainer_shap.expected_value[1],
                           ...
                           # affichage des 15 variables les + importantes
...
shap_values = explainer_shap.shap_values(
    X_test_clean().drop(labels="sk_id_curr", axis=1))
shap_values_df = pd.DataFrame(data=shap_values[1], columns=X_test_clean().drop(
    labels="sk_id_curr", axis=1).columns)

df_groupes = pd.concat(
    [y_pred_lgbm_proba_df['proba_classe_1'], shap_values_df], axis=1)
...
df_groupes_mean = df_groupes_mean.rename_axis(
    'typologie_clients').reset_index()
...
st_shap(shap.decision_plot(explainer_shap.expected_value[1],
    ...
    feature_order='importance',
    highlight=0,
    legend_labels=['Client', '20%_et_moins', '21%_30%',
                   '31%_40%', '41%_60%', '61%_et_plus'],
...
liste_variables = X_test_clean().drop(
...
#def load_customer_score(data, customer_id, model):
    ...
    #customer_score = model.predict_proba(
        #data[data.index == int(customer_id)])
    #return customer_score[0]


#def get_score_credit():
    #score = load_customer_score(initial_data,ID_client, model_LGBM)
    #return {"score ": score*100}

#score_1=get_score_credit()
...
st.plotly_chart(px.histogram(new_df_2, y="ext_source_2",
                x='sk_id_curr', color="score"))
